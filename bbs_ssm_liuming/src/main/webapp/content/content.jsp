...
<body>
	...
	<div class="container">
		<div class="row">
			<!--左边板块-->
			<div id="content_left" class="col-xs-7 col-md-7 bottom_left">
				<!--代码部分begin-->
				<div class="jq22">
					<div class="hidden">
						<!--此处进行循环展示-->
							...
							<c:if test="${listArticle.status==1 }">
								<div class="row show_article">
									<div class="col-md-12 show_article_go">
										...
										<div class="row">
											<div class="col-md-12">
												<a class="bottom_left_aaa a_b" href="${APP_PATH}/articleController/getArticleFid?fid=${listArticle.fid}">
													<h4>
														<!-- 帖子标题 -->
										...
										<c:forEach var="comment" items="${map[listComment_Fid]}">
			...
			<!--右边板块-->
			<div id="content_right" class="col-xs-3 col-md-3 bottom_right">
				<div class="row">
					<div class="col-md-12"
						style="position: relative; padding-top: 10px;">
						<b>所有板块</b>
				...
				<c:forEach var="listArticle" items="${listArticle }">
					<!-- 只显示通过审核的帖子且评论数大于2文章 -->
					<c:if test="${listArticle.status==1 && listArticle.sum>2 }">
						<div class="row">
							<div class="col-md-12">
...
<script>
	$(function () {
		var plate = eval(${plate_str});
		var plateDiv = $("#plate_div");
        for (var i = 0; i < plate.length; i++) {
			var inhtml= "\t\t<div class=\"col-md-12 plate_cdiv\">\n" +
                "\t\t\t\t\t\t<a href=\"${APP_PATH }/common/getArticleBname?bname="+plate[i].bname+"\">\n" +
                "\t\t\t\t\t\t\t<button type=\"button\" class=\"btn btn-default btn-lg btn-block plate_btn\">\n" +
                "\t\t\t\t\t\t\t\t<span class=\"glyphicon glyphicon-tags plate_sp\" aria-hidden=\"true\"></span>\n" +
                "\t\t\t\t\t\t\t"+plate[i].bname+"</button>\n" +
                "\t\t\t\t\t\t</a>\n" +
                "\t\t\t\t\t</div>"
			plateDiv.append(inhtml);
        }
    })
</script>
</html>