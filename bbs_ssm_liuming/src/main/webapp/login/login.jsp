...
<body>
	...
	$("#login").submit(function(){
		...
		$(function () {
			$("#name2").blur(checkName);
			$("#repassword").blur(checkPassword);
			$("#email").blur(checkEmail);
            $("#name2").change(function () {
				$.get("${APP_PATH}/userController/getUserName",{"name":""+$(this).val()+""},function (data) {
				    if (data.trim() == "OK") {
                        $("#registerName").html("<span style='color: green'>恭喜你，该用户名可用</span>");
                    }else {
                        $("#registerName").html("<span style='color: red'>对不起，该用户名已存在</span>");
					}
                });
            });
        });
